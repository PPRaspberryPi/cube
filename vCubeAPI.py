...
def ball_on(target_location):
    ...
    y_center = round(y)
    z_center = round(z)
    cube.buffer_cubes[((x_center % cubeSize) + ((y_center % cubeSize) * cubeSize) + (
            (z_center % cubeSize) * (cubeSize ** 2)))].setOn()


def ball_off(target_location):
    x = target_location[0] * (cubeSize - 1)
    y = target_location[1] * (cubeSize - 1)
    z = target_location[2] * (cubeSize - 1)
    x_center = round(x)
    ...
    z_center = round(z)
    cube.buffer_cubes[((x_center % cubeSize) + ((y_center % cubeSize) * cubeSize) + (
            (z_center % cubeSize) * (cubeSize ** 2)))].setOff()
